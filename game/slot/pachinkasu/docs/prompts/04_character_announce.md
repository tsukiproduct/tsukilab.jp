...
**液晶ストリップ内に収まる形式**なので、縦長で作ると文字が小さく潰れて窮屈になる。
**必ず横長で、キャラクターを画面の片側に寄せ、大きな文字がもう半分を占める構図**にする。
中央に全身を置いて下に小さく文字を置く縦長ポスターの構図は流用できない。

### 推奨サイズ: **1200 × 800 px（3:2）**

液晶の縦横比は端末によって変わる。`#lcd` は `flex:0 0 33%` で、`#app` の
`max-height:230vw` に当たるかどうかで高さが決まるため——

| 端末 | 液晶の実測 | 比 |
|---|---|---|
| iPhone SE 375×667 | 374×220 | 1.70:1 |
| iPhone X/13 375×812 | 373×268 | **1.39:1** |
| Pixel 360×800 | 360×264 | 1.36:1 |
| iPad 768×1024 | 573×338 | 1.70:1 |
| PC 1920×1080 | 605×356 | 1.70:1 |

**1.36:1 〜 1.70:1 の範囲で動く。** `#announce` は `object-fit:contain` なので
どの比率でも画像が切れることはなく、余った側が黒帯になる（実機の液晶と同じ見え方）。
黒帯を最小にするには両端の相乗平均 √(1.36×1.70) ≒ **1.52 ＝ 3:2** が最適で、
このとき黒帯は最大でも 12% に収まる。

- **サイズ: 1200×800 px（3:2）**
- **セーフエリア: 中央 88% × 82%**。文字・顔・ロゴはこの内側に収める
  （どちらの比率でも確実に見える範囲。外側は黒帯で隠れる可能性がある）
- 既存の 900×563（1.6:1）も切れずに出るので作り直しは不要。新規は 3:2 で作る
- 背景の端は黒〜暗色にしておくと黒帯との境目が目立たない
...
強カットイン(`cut_s`)はボーナス確定演出(`#revealCut`）でも使い回す。そちらは
液晶の上半分に幅86%・高さ最大52%の帯として置かれ、下に「BONUS 濃厚」の文字が入る。
**1コマあたり 1540×700 px（2.2:1）**を目安に、顔が帯の中央に来る構図にすること。
