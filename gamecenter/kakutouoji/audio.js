...
window.FightAudio=(()=>{
 ...
 const music=new Audio('assets/audio/battle.mp3');music.loop=true;music.preload='auto';music.volume=.06;
 ...
 function syncMusic(){if(scene&&enabled&&musicEnabled&&unlocked){music.volume=performance.now()<duckUntil?.025:.06;if(music.paused)music.play().catch(()=>{})}else music.pause()}
 function stop(){for(const n of nodes){try{n.stop()}catch(e){}}nodes.clear()}
 function setScene(v){scene=v;if(v!==lastScene){lastScene=v;if(!v)stop();syncMusic()}else if(v&&musicEnabled)music.volume=performance.now()<duckUntil?.025:.06}
